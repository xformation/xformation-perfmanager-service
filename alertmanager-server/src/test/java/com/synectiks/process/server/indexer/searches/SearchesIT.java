...
public abstract class SearchesIT extends ElasticsearchBaseTest {
    ...
    public void determineAffectedIndicesWithRangesIncludesDeflectorTarget() throws Exception {
        ...
        final MongoIndexRange indexRange0 = MongoIndexRange.create("alertmanager_0", now, now.plusDays(1), now, 0);
        final MongoIndexRange indexRange1 = MongoIndexRange.create("alertmanager_1", now.plusDays(1), now.plusDays(2), now, 0);
        final MongoIndexRange indexRangeLatest = MongoIndexRange.create("alertmanager_2", new DateTime(0L, DateTimeZone.UTC), new DateTime(0L, DateTimeZone.UTC), now, 0);
    ...
    public void determineAffectedIndicesWithRangesExcludeEvents() throws Exception {
        ...
        final MongoIndexRange indexRange2 = MongoIndexRange.create("alertmanager_0", now, now.plusDays(1), now, 0);
        final SortedSet<IndexRange> indices = ImmutableSortedSet.orderedBy(IndexRange.COMPARATOR)
                .add(indexRange0)
                .add(indexRange1)
                .add(indexRange2)
    ...
    public void scrollReturnsResultWithSelectiveFields() throws Exception {
        ...
        when(indexSetRegistry.getForIndices(Collections.singleton("alertmanager_0"))).thenReturn(Collections.singleton(indexSet));
        final AbsoluteRange range = AbsoluteRange.create(new DateTime(2015, 1, 1, 0, 0, DateTimeZone.UTC).withZone(UTC), new DateTime(2015, 1, 2, 0, 0, DateTimeZone.UTC).withZone(UTC));
