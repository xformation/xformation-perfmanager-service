...
class ParsedElasticsearchExceptionTest {
    ...
    void parsingPrimaryShardIsNotAvailable() {
        final String exception = "Elasticsearch exception: ElasticsearchException[Elasticsearch exception [type=unavailable_shards_exception, " +
                "reason=[alertmanager_0][2] primary shard is not active Timeout: [1m], request: [BulkShardRequest [[alertmanager_0][2]] containing [125] " +
                "requests]]]";
        ...
        assertThat(parsed).satisfies(p -> {
            ...
            assertThat(p.reason()).isEqualTo("[alertmanager_0][2] primary shard is not active Timeout: [1m], " +
                    "request: [BulkShardRequest [[alertmanager_0][2]] containing [125] requests]]");
        });
    }
}
